...
class SpecDecDReSD(torch.nn.Module):
    ...
    def __init__(self, big_model, small_model):
        ...
        assert big_model.device == small_model.device
    ...
    def speculative_decoding(
        ...
        while self.big_model._has_unfinished_sequences(this_peer_finished, False, device=input_ids.device):
            ...
            candidate_input_ids = candidate_generator.get_candidates(input_ids, big_embedding)
            ...
            if 'past_key_values' in model_kwargs:
                model_kwargs['past_key_values'] = [tuple(layer[i].repeat(candidate_input_ids.size(0), 1, 1, 1) for i in [0, 1])
            ...
            big_embedding = outputs.hidden_states[-1][match_id][-(candidate_length - n_matches + 1)]
    ...
    def to(self, device):
        self.big_model = self.big_model.to(device)
...
class SpeculativeCandidateGenerator:
    ...
    def get_candidates(self, input_ids: torch.Tensor, big_embedding: torch.Tensor) -> torch.Tensor:
        ...
        assistant_output = self.assistant_model.generate(big_embedding, **assistant_generation_kwargs)
