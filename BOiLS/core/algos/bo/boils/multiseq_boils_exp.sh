...
circuits=()  # 46 - 2
circuits=()  # 46 - 2
#circuits=(square log2 multiplier sqrt adder max)  # 46 - 2
circuits=(hyp adder max sin square sqrt multiplier div log2)
circuits=(hyp adder max sin square multiplier div log2)
...
#        if ((device_to_export >= 0)); then export CUDA_VISIBLE_DEVICES=$device_to_export; fi
        ...
        cmd="python ./core/algos/bo/boils/main_multi_boils.py \
                      ...
                      --objective $objective \
                      --seed $seed $overwrite"
        CUDA_VISIBLE_DEVICES=$device_to_export taskset -c $taskset_start-$taskset_end $cmd &
        echo $cmd
      done
    done
  wait
