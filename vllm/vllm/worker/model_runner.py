...
class ModelRunner(GPUModelRunnerBase[ModelInputForGPUWithSamplingMetadata]):
    ...
    def execute_model(
        ...
        if self.return_hidden_states:
            ...
            if hasattr(self.model, 'post_process_KV') and kv_caches[0].shape[0] > 0:
                tot_entries = kv_caches[0].shape[1] * kv_caches[0].shape[2]
                num_head, head_size = kv_caches[0].shape[3], kv_caches[0].shape[4]
                tot_seq_len = model_input.attn_metadata.slot_mapping.shape[0]

                KV = torch.stack([kv_caches[i].view(2, tot_entries, num_head, head_size)[:, model_input.attn_metadata.slot_mapping].permute(1, 2, 0, 3).reshape(tot_seq_len, -1) for i in range(len(kv_caches))], 1)
                output.post_process_KV = self.model.post_process_KV(KV)
