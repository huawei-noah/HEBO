...
class HiddenStates(msgspec.Struct, array_like=True,
    ...
    previous_slot_mapping: Optional[torch.Tensor] = None
    ...
    def update(self,
        ...
        if torch.is_tensor(self.hidden_states):
            self.hidden_states = torch.cat([self.hidden_states, hidden_states])
        else:
            self.hidden_states.extend([hidden_states[i:i+1] for i in range(hidden_states.shape[0])])
    ...
    def prune(self,
        ...
        if seq_ids != self._seq_ids:
            ...
            self.hidden_states = self.hidden_states[index] if torch.is_tensor(self.hidden_states) else [self.hidden_states[i] for i in index]
            if self.second_last_token_hidden_states is not None:
                self.second_last_token_hidden_states = self\
                    .second_last_token_hidden_states[index]
            if self.previous_slot_mapping is not None:
                self.previous_slot_mapping = self.previous_slot_mapping[:, index]
