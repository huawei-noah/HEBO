...
class TP1DraftModelRunner(ModelRunner):
    ...
    def execute_model(
        ...
        previous_slot_mapping: Optional[IntermediateTensors] = None,
        num_steps: int = 1,
        ...
        kwargs = {}
        force_token_remap = False
        if (model_input.attn_metadata.decode_metadata is not None and
                self.model_config.hf_config.model_type == 'moa_spec'):
            attn_metadata_ca = copy.deepcopy(model_input.attn_metadata)
            attn_metadata_ca.decode_metadata.seq_lens_tensor -= 1

            remap = []
            remap2 = []
            needs_rewrite = []
            ind = 0
            while ind < len(model_input.sampling_metadata.seq_groups):
                cur_seq_id = model_input.sampling_metadata.seq_groups[ind].seq_ids[0]
                prev_seq_id = model_input.sampling_metadata.seq_groups[ind - 1].seq_ids[0] if ind > 0 else None
                next_seq_id = model_input.sampling_metadata.seq_groups[ind + 1].seq_ids[0] if ind + 1 < len(model_input.sampling_metadata.seq_groups) else None

                if cur_seq_id != prev_seq_id:
                    remap.append(ind)
                if cur_seq_id != next_seq_id:
                    remap2.append(ind)

                force_token_remap |= cur_seq_id == next_seq_id
                needs_rewrite.append(cur_seq_id == next_seq_id)

                ind += 1

            if previous_hidden_states is not None and previous_slot_mapping is not None and previous_slot_mapping.shape[1] > 0:
                if force_token_remap:
                    attn_metadata_ca.slot_mapping = torch.stack([previous_slot_mapping[k, i]
                                                                 if k < previous_slot_mapping.shape[0] else
                                                                 attn_metadata_ca.slot_mapping[remap[i]]
                                                                 for i, t in enumerate(previous_hidden_states) for k in
                                                                 range(t.shape[0])])

                    previous_positions = torch.cat(
                        [model_input.input_positions[remap2[i]] - torch.arange(t.shape[0], 0, -1, device=model_input.input_positions.device) for i, t in
                         enumerate(previous_hidden_states)])
                else:
                    attn_metadata_ca.slot_mapping = torch.stack(
                        [previous_slot_mapping[k, i] for i, t in enumerate(previous_hidden_states) for k in
                         range(t.shape[0])])
                    previous_positions = torch.cat(
                        [model_input.input_positions[i] - torch.arange(t.shape[0], 0, -1, device=model_input.input_positions.device) for i, t in
                         enumerate(previous_hidden_states)])
    
                kwargs["previous_positions"] = previous_positions

            kwargs["attn_metadata_ca"] = attn_metadata_ca
        self.my_timer += time.time() - start_time
        ...
        for step in range(num_steps):
            ...
            if previous_hidden_states is not None:
                ...
                if step == 1 and self.model_config.hf_config.model_type == 'moa_spec':
                    previous_hidden_states = None
                    del kwargs["previous_hidden_states"]
            ...
            start_time = time.time()
            if (self.model_config.hf_config.model_type == 'moa_spec'
                    and model_input.attn_metadata.decode_metadata is not None):
                if force_token_remap:
                    outputs[-1].previous_slot_mapping = model_input.attn_metadata.slot_mapping[remap]
                else:
                    outputs[-1].previous_slot_mapping = copy.deepcopy(model_input.attn_metadata.slot_mapping)

            # Bonus token has weird behavior on step > 1
            # Rewrite token sampling
            if force_token_remap:
                for i, v in enumerate(needs_rewrite):
                    if v:
                        outputs[-1].sampled_token_ids[i][:] = model_input.input_tokens[i + 1]
            self.my_timer += time.time() - start_time
