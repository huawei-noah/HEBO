...
class SpecDecodeWorker(LoraNotSupportedWorkerBase):
    ...
    * The scoring forward pass is done without an MQA kernel, which is
    ...
    def __del__(self):
        import numpy as np
        print(f"{np.mean(self.accept_len)=}")
    ...
    def init_device(self) -> None:
        ...
        self.is_moa = False
        if self.proposer_worker.model_config.hf_config.model_type == "moa_spec":
            self.proposer_worker.model_runner.model.share_weights(self.scorer_worker.model_runner.model)
            self.is_moa = True
            self.LSA_enabled = not self.proposer_worker.model_config.hf_config.disable_LSA
    ...
    def _run_no_spec(self, execute_model_req: ExecuteModelRequest,
        ...
        if hidden_states is not None and not self.is_moa:
        ...
        if not skip_proposer:
            ...
            if self.is_moa:
                small_model_input = (sampler_output.post_process_KV
                                        if self.LSA_enabled else sampler_output.prefill_hidden_states
                )

                execute_model_req.previous_hidden_states = prepare_prefill_hidden_states(small_model_input, roll=False)
            else:
    ...
    def _run_speculative_decoding_step(
        ...
        self.accept_len.append((accepted_token_ids != -1).sum(-1).half().mean().item())
    ...
    def _verify_tokens(
        ...
        if self.is_moa:
            nb_valid_tokens = (accepted_token_ids + 1).count_nonzero(dim=1)
            if self.LSA_enabled:
                assert proposal_scores.post_process_KV.shape == hidden_states.shape
                hidden_states = proposal_scores.post_process_KV
                hidden_states = [hidden_states[i, :v] for i, v in enumerate(nb_valid_tokens)]
            else:
                hidden_states = [hidden_states[i, :v] for i, v in enumerate(nb_valid_tokens)]
            self.previous_hidden_states = (
                HiddenStates(hidden_states,
                             seq_group_metadata_list,
                             previous_slot_mapping=proposals.previous_slot_mapping)
            )
...
def prepare_prefill_hidden_states(
        prefill_hidden_states: torch.Tensor, roll: bool = True) -> HiddenStates:
    ...
    return HiddenStates(prefill_hidden_states) if prefill_hidden_states is not None else None
