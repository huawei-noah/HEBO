...
from pymoo.core.mixed import MixedVariableMating, MixedVariableGA, MixedVariableDuplicateElimination
...
from ..utils.constr_utils import eval_constraint
from ..design_space.design_space import DesignSpace
from ..acquisitions.acq import Acquisition


def space_to_pymoo_vars(space) -> Dict[str, Union[Real, Integer, Choice]]:
    variables = {}
    for i, p_name in enumerate(space.para_names):
        ...
        if p.is_numeric:
            if not p.is_discrete_after_transform:
                variables[p_name] = Real(bounds=(lb, ub))
            else:
                variables[p_name] = Integer(bounds=(lb, ub))
        elif p.is_categorical:
            variables[p_name] = Choice(options=list(range(int(lb), int(ub + 1))))
        else:
...
class BOProblem(Problem):
    def __init__(self, acq: Acquisition, space: DesignSpace, input_constraints: List[str], fix: dict = None):
        self.acq = acq
        self.space = space
        self.fix = fix  # NOTE: use self.fix to enable contextual BO
        self.input_constraints = input_constraints
        variables = space_to_pymoo_vars(self.space)
        super().__init__(vars=variables, n_obj=acq.num_obj, n_constr=acq.num_constr + len(input_constraints))
    ...
    def _evaluate(self, para: np.ndarray, out: dict, *args, **kwargs):
        ...
        for n in self.space.numeric_names:
            func = np.vectorize(lambda x_entry: x_entry[n])
            x.append(func(para))
        ...
        out['G'] = np.zeros((len(para), self.acq.num_constr + len(self.input_constraints)))

        out['G'][:, :self.acq.num_constr] = acq_v[:, self.acq.num_obj:].numpy()
        for i, input_constraint in enumerate(self.input_constraints):
            val = []
            for p in para:
                val.append(eval_constraint(para=p, constr=input_constraint))
            out["G"][:, self.acq.num_constr + i] = np.array(val)
...
class EvolutionOpt:
    ...
    def optimize(self, initial_suggest: pd.DataFrame = None, fix_input: dict = None, return_pop=False) -> pd.DataFrame:
        prob = BOProblem(acq=self.acq, space=self.space, input_constraints=self.input_constraints, fix=fix_input)
        init_pop = get_init_pop(
            space=self.space, pop=self.pop, initial_suggest=initial_suggest, sobol_init=self.sobol_init
        )
