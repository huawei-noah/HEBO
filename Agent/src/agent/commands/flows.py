...
from agent.utils import break_word_split, extract_action_as_json
from agent.utils.utils import extract_command_as_json, extract_metric_as_json
from agent.utils.utils import extract_instruction_as_json
from agent.utils.utils import extract_paths_as_json
from agent.utils.utils import extract_submission_as_json
from agent.utils.utils import extract_summary_as_json
from agent.utils.utils import extract_detected_file_as_json
...
PARSE_FUNC_MAP = {
    ...
    "extract_paths_as_json": extract_paths_as_json,
    "extract_instruction_as_json": extract_instruction_as_json,
    "extract_detected_file_as_json": extract_detected_file_as_json,
...
class DecisionFlow(Flow):
    ...
    def __init__(
            ...
            memory_choice_tag_val: str | None = None,
    ...
    def step(self, agent) -> Command | Flow | None:
        ...
        if len(choices) == 1:
            ...
            if agent.llm_answers is not None and len(agent.llm_answers) > 0 and agent.llm_answers[0] == output:
                agent.llm_answers.pop(0)  # skip this answer as LLM won't be querried
        else:
