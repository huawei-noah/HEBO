...
class HumanTakeoverCommand(Command):
    trigger_human_takeover: bool = False
    ...
    def check_trigger_human_takeover(self) -> bool:
        if self.trigger_human_takeover:
            return True

        if hasattr(self, "human_takeover"):
...
class UseTool(Command):
    ...
    def __call__(self, agent):
        ...
        if self.tool.requires_llm_prompt:
            ...
            tool_output = self.tool(agent_input=tool_input)
        for tag in tool_output:
            agent.memory.store(tool_output[tag], {tag})


class Break(Command):
    name: str = "break"
    description: str = "Break a LoopFlow."
