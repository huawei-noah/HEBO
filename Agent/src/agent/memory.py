...
class MemKey(str, ListableEnum):
    ...
    TRIGGER_HUMAN_TAKEOVER = "_trigger_human_takeover"
    ...
    BO_RAN = "bo_ran"
    BO_ERROR = "bo_error"
    BO_OBSERVATIONS = "bo_observations"
...
class Memory:
    ...
    def store(self, content: Any, tags: MemKey | list[MemKey] | Set[MemKey]) -> None:
        ...
        assert tags.issubset(set(self.mem_keys)), (
            f"All stored tags must be valid memory keys (MemKey enum). If you are intentionally creating new keys, "
            f"please add them to the memory keys enum. TAGS: {tags}"
        )
