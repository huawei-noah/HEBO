...
class GPTNeoXAttention(nn.Module):
    ...
    def forward(
        ...
        h_attn = None if hooks is None else hooks.attn
        query, key, value, present = self._attn_projections_and_rope(
            hidden_states=hidden_states, position_ids=position_ids, layer_past=layer_past, use_cache=use_cache, hooks=h_attn
        )

        # Compute attention
        attn_output, attn_weights = self._attn(query, key, value, attention_mask, head_mask, hooks=h_attn)
    ...
    def _attn_projections_and_rope(
        ...
        if hooks is not None:
            query = hooks.post_rota_emb_q(query)
            key = hooks.post_rota_emb_k(key)
    ...
    def _attn(self, query, key, value, attention_mask=None, head_mask=None, hooks=None):
        ...
        if hooks is not None:
            attn_weights = hooks.qk_probs(attn_weights)

        attn_output = torch.matmul(attn_weights, value)
        if hooks is not None:
            attn_output = hooks.v_out(attn_output)
...
class GPTNeoXFlashAttention2(GPTNeoXAttention):
    ...
    def forward(
        ...
        if hooks is not None:
            query = h_attn.q(query)
            key = h_attn.k(key)
            value = h_attn.v(value)
        ...
        if hooks is not None:
            attn_weights = hooks.attn.qk_probs(attn_weights)
        ...
        if hooks is not None:
            attn_output = hooks.attn.v_out(attn_output)
...
class GPTNeoXSdpaAttention(GPTNeoXAttention):
    ...
    def forward(
        ...
        if output_attentions or head_mask is not None or hooks is not None:
            logger.warning_once(
        ...
        if hooks is not None:
            query = hooks.attn.q(query)
            key = hooks.attn.k(key)
            value = hooks.attn.v(value)
...
class GPTNeoXMLP(nn.Module):
    ...
    def forward(self, hidden_states, hooks: MLPHooks = MLPHooks()):
        hidden_states = self.dense_h_to_4h(hidden_states)
        if hooks is not None:
            hidden_states = hooks.pre_act(hidden_states)
        hidden_states = self.act(hidden_states)
        if hooks is not None:
            hidden_states = hooks.post_act(hidden_states)
...
class GPTNeoXLayer(nn.Module):
    ...
    def forward(
        ...
        inp_ln = self.input_layernorm(hidden_states)
        if hooks is not None:
            inp_ln = hooks.resid.torso.input_ln_attn(inp_ln)
        ...
        if hooks is not None:
            attn_output = hooks.resid.torso.delta_attn(attn_output)

        if self.use_parallel_residual:
            # pseudocode:
            # x = x + attn(ln1(x)) + mlp(ln2(x))
            post_ln_attn = self.post_attention_layernorm(hidden_states)
            hmlp = None
            if hooks is not None:
                post_ln_attn = hooks.resid.torso.post_ln_attn(post_ln_attn)
                hmlp = hooks.mlp
            mlp_output = self.mlp(post_ln_attn, hmlp)
            mlp_output = self.post_mlp_dropout(mlp_output)
            if hooks is not None:
                mlp_output = hooks.resid.torso.delta_mlp(mlp_output)
            hidden_states = mlp_output + attn_output + hidden_states
            if hooks is not None:
                hidden_states = hooks.resid.torso.post_mlp(hidden_states)
        else:
            # pseudocode:
            # x = x + attn(ln1(x))
            # x = x + mlp(ln2(x))
            ...
            hmlp = None
            if hooks is not None:
                attn_output = hooks.resid.torso.post_attn(attn_output)
                hmlp = hooks.mlp
            post_ln_attn = self.post_attention_layernorm(attn_output)
            if hooks is not None:
                post_ln_attn = hooks.resid.torso.post_ln_attn(post_ln_attn)
            mlp_output = self.mlp(post_ln_attn, hmlp)
            mlp_output = self.post_mlp_dropout(mlp_output)
...
class GPTNeoXModel(GPTNeoXPreTrainedModel):
    ...
    def forward(
        ...
        if hooks is not None:
            hidden_states = hooks.resid.post_emb(hidden_states)
        ...
        for i, (layer, layer_past) in enumerate(zip(self.layers, past_key_values)):
            hooks_layer_i = None if hooks is None else deepcopy(hooks).bind(layer=i)
        ...
        if hooks is not None:
            hidden_states = hooks.resid.post_ln_f(hidden_states)
