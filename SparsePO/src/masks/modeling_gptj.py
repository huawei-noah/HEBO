...
class GPTJAttention(nn.Module):
    ...
    def forward(
        ...
        if hooks is not None:
            _ = h_attn.q(self._split_heads(pre_q, self.num_attention_heads, self.head_dim, False))
            _ = h_attn.k(self._split_heads(pre_k, self.num_attention_heads, self.head_dim, False))
        ...
        if hooks is not None:
            query = h_attn.post_rota_emb_q(query)
            key = h_attn.post_rota_emb_k(key)
        ...
        if hooks is not None:
            attn_weights = h_attn.qk_probs(attn_weights)
        ...
        if hooks is not None:
            attn_output = h_attn.v_out(attn_output)
...
class GPTJMLP(nn.Module):
    ...
    def forward(self, hidden_states: Optional[torch.FloatTensor], hooks: MLPHooks = MLPHooks()) -> torch.FloatTensor:
...
class GPTJBlock(nn.Module):
    ...
    def forward(
        ...
        feed_forward_hidden_states = self.mlp(hidden_states, hmlp)
        if hooks is not None:
