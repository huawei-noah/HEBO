...
class HookCollection:
    ...
    def append_to_path(self, hook_location_name, hook):
        ...
        if len(hook_location_parts) == 1:  # no dot in path
            self.all_hooks[top_level_location].append_all(hook)
...
class AttentionHooks(HookCollection):
    def __init__(self):
        ...
        self.add_subhooks("post_rota_emb_q", FwdBwdHooks())
        self.add_subhooks("post_rota_emb_k", FwdBwdHooks())
...
class ResidualStreamTorsoHooks(HookCollection):
    def __init__(self):
        super().__init__()
        self.add_subhooks("input_ln_attn", FwdBwdHooks())
