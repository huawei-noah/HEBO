...
class BODi(BoBase):
    ...
    def get_name(self, no_alias: bool = False) -> str:
        ...
        # Check the cases when BODi name applies
        if not no_alias and not self.use_tr:
            return "BODi"

        tr_prefix = "Tr-based " if self.use_tr else ""

        nominal_kernel_name = f"HED-{self.model_cat_hed_base_kernel_name}"

        if self.is_mixed:
            name = f'GP ({self.model_numeric_kernel_name} and ' \
                   f'{nominal_kernel_name}) - {tr_prefix}IS acq optim'
        elif self.is_numeric:
            name = f'GP ({self.model_numeric_kernel_name}) - {tr_prefix}IS acq optim'
        elif self.is_nominal:
            name = f'GP ({nominal_kernel_name}) - {tr_prefix}IS acq optim'
        else:
            raise ValueError()
        return name
    ...
    def __init__(self,
                 ...
                 model_cat_hed_base_kernel_name: str = 'mat52',
                 model_cat_hed_base_kernel_ard: bool = True,
                 model_cat_hed_num_embedders: int = 128,
        ...
        assert search_space.num_cont + search_space.num_disc + search_space.num_nominal == search_space.num_dims, \
            'BODi algorithm only supports continuous, discrete and nominal variables'
        ...
        self.model_cat_hed_base_kernel_name = model_cat_hed_base_kernel_name
        self.model_cat_hed_num_embedders = model_cat_hed_num_embedders
        ...
        cat_hed_base_kernel = kernel_factory(
            kernel_name=self.model_cat_hed_base_kernel_name,
            active_dims=np.arange(self.model_cat_hed_num_embedders),
            use_ard=model_cat_hed_base_kernel_ard,
        ...
        nominal_kernel_kwargs = {
            ...
            "hed_num_embedders": self.model_cat_hed_num_embedders
        }

        # in the paper they do simply a product kernel, while in the code the concatenate the embedding of nominal
        # dims with the continuous dims and use a MaternKernel on the resulting vector.
        kernel = mixture_kernel_factory(
            ...
            nominal_kernel_name="hed",
            nominal_kernel_use_ard=False,
